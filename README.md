...
The biggest difference comes into place when we want to plot randomly selected examples fomr correctly and misclassified samples. While in the case of the RF classifier setting we have to reconstruct the image from a flat vector, the CNN version requires different handling due to tensor-based image storage and normalization. Here the images are stored as tensors in an array. Thereby the reshaping as in RF can be skipped. But we have to denormalize the image data as our CNN was trained with normalized inputs. This is not the case with our RF. Therefore a reversing of the normalized image data is only necessary in the CNN case. It is done by mutliplying the image data with the standard deviation and subsequently adding the mean of the whole test dataset. To ensure we do not run into any errors or warning, the resulting image data is clipped to the range of 0 to 255. Otherwise it could be possible that we encounter invalid values after floating-point operations. In the next step, we transpose our image data in tensor format as PyTorch and Matplotlib are using and expecting different shapes. Lastly, we drop the NIR channel as Matplotlib can only work with the RGB channels. Because our image data is in float format after denormalization, Matplotlib expect and interprets it in the [0,1] range. Therefore we have to divide them by 255.             
